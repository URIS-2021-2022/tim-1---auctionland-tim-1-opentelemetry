using DocumentMsProject.Models;
using Microsoft.Extensions.Configuration;
using Newtonsoft.Json;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Net.Http;
using System.Threading.Tasks;

namespace DocumentMsProject.ServiceCalls.CustomerMicroservice
{
    public class CustomerMicroservice : ICustomerMicroservice
    {
        private readonly IConfiguration configuration;

        public CustomerMicroservice(IConfiguration configuration)
        {
            this.configuration = configuration;
        }
        public bool GetCustomers(CustomerDto customerDto)
        {
            using (HttpClient client = new HttpClient())
            {
                try
                {
                    Uri url = new Uri($"{ configuration["Services:CustomerMicroservice"] }api/customers");

                    HttpContent content = new StringContent(JsonConvert.SerializeObject(customerDto));
                    content.Headers.ContentType.MediaType = "application/json";

                    HttpResponseMessage response = client.PostAsync(url, content).Result;
                    if (!response.IsSuccessStatusCode)
                    {
                        return false;
                    }
                    return true;
                }
                catch
                {
                    return false;
                }
            }
        }
    }
}
